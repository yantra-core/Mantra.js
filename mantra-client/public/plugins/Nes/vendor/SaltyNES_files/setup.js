	...
	documentOnReady(() => {
		downloadAndLoadScript("index.wasm", "application/octet-binary", function() {
				downloadAndLoadScript("static/index.js", "text/javascript", function() {
						...
						// TODO: auto loading game seems to miss some event bind, enter not working
						/*
						setTimeout(function() {
						// After everything is set up, automatically load the first game
						let selectGame = document.getElementById('select_game');
						console.log('selectGame', selectGame, selectGame.value)
						if (selectGame && selectGame.options.length > 1) {
								selectGame.selectedIndex = 4; // Select the first game
								selectGame.dispatchEvent(new Event('change')); // Trigger the change event
						}
						}, 1000);
						*/
				});
