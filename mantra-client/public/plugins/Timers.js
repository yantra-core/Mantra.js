...
var Timers = /*#__PURE__*/function (_Plugin) {
  ...
  _createClass(Timers, [{
    ...
    key: "createTimer",
    value: function createTimer(timerName, duration) {
      var isInterval = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
      var timer = new _TimersComponent["default"](timerName, duration, isInterval);
          ...
          Object.keys(timersComp.timers).forEach(function (timerName) {
            ...
            if (!timer.completed && Date.now() >= timer.startTime + timer.duration) {
              ...
              if (timer.isInterval) {
                ...
                _this2.game.emit('timer::done', entity, timerName, timer);
              } else {
                timer.completed = true;
                _this2.game.emit('timer::done', entity, timerName, timer);
              }
              if (_this2.game.data) {
                // if data.timer && data.
                _this2.game.data.timers = _this2.game.data.timers || [];
                _this2.game.data.timers.push({
                  entityId: entity.id,
                  entity: entity,
                  gameType: 'timer',
                  timerName: timerName,
...
},{"../../Component/TimersComponent.js":2,"../../Plugin.js":3}]},{},[4])(4)
});
