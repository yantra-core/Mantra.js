...
var Sutra = /*#__PURE__*/function () {
  function Sutra() {
    ...
    this.id = Sutra.id;
    console.log('sutrasutrasutra', _index["default"]);
    this.sutra = _index["default"];
  }
  _createClass(Sutra, [{
    key: "init",
    value: function init(game) {
      this.game = game;
      this.game.createSutra = _index["default"].createSutra;
  ...
  _createClass(Sutra, [{
    ...
    value: function traverseNode(node, data, gameState) {
      if (node.subtree) {
        ...
        if (subSutra) {
          ...
          var conditionMet = node["if"] ? this.evaluateCondition(node["if"], data, gameState, subSutra) : true;
          if (conditionMet) {
      ...
      if (node.action) {
        ...
        var _conditionMet = node["if"] ? this.evaluateCondition(node["if"], data, gameState) : true;
        if (_conditionMet) {
    ...
    value: function processBranch(branch, data, gameState) {
      ...
      if (Array.isArray(branch)) {
        branch.forEach(function (childNode) {
          return _this5.traverseNode(childNode, data, gameState);
        });
      }
    }
  }, {
    ...
    value: function tick(data) {
      ...
      this.tree.forEach(function (node) {
        return _this7.traverseNode(node, data, gameState);
      });
    }
  }, {
