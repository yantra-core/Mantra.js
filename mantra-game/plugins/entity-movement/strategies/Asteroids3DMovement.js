...
class AsteroidsMovementStrategy {
  ...
  update(entityId, dx, dy, dz) {
    ...
    // Calculate the direction vector based on the body's rotation
    // Assuming the body faces towards the positive y-axis when pitch and yaw are 0
    ...
    const force = {
      x: forwardDirection.x * dy * this.thrust + dx * this.thrust, // Forward/backward thrust along the x-axis
      y: forwardDirection.y * -dy * this.thrust, // Forward/backward thrust along the y-axis
